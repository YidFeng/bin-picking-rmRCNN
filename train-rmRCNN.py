...
if __name__ == '__main__':
    ...
    # a = cv2.imread('/home/dan/Desktop/bin-picking/dataset/07/train/gray/0.png')
    # tt = EraseTransform(p=0.5, scale=(0.001, 0.05), ratio=(0.1,5))
    # tt = T.RandomBrightness(3, 3.1).get_transform(a)
    # tt = T.RandomContrast(2, 2.1).get_transform(a)
    # tt = T.ColorTransform(lambda x: (random_noise(x, mode='pepper', amount=0.08, clip=True)*255).astype('uint8'))


    # cv2.imwrite('../pepper0.08.png',b)
    # tt = T.RandomContrast(0.5, 0.51).get_transform(a)
    # for i in range(10):
    #     tt = T.RandomExtent(scale_range=(0.3, 1), shift_range=(1, 1)).get_transform(a)
    #     b = tt.apply_image(a)
    #     plt.figure()
    #     plt.imshow(b)
    #     plt.show()

    #     tt = RandomPepperNoise().get_transform(a)
    #     b = tt.apply_image(a)
    #     cv2.imshow('b', b)
    # # cv2.imshow('a', a)
    #     cv2.waitKey(0)
